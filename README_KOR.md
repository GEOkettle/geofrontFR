...
geofrontFR는 TanStack Start를 주 도메인 백엔드가 아니라 SSR 런타임으로 사용한다.

기본 서버 경계는 아래와 같다.

- TanStack Start server: SSR 렌더링, 요청 단위 auth 해석, 쿠키/헤더 읽기, private 환경 변수 접근, 최소한의 server-only orchestration
- External API server: 도메인 비즈니스 로직, 영속 데이터 접근, 인증 API, 비즈니스 성격의 mutation/query

규칙:

- 기본 원칙으로 핵심 도메인 비즈니스 로직을 TanStack Start server 안으로 옮기지 않는다.
- `server/` 레이어를 두 번째 풀백엔드 애플리케이션처럼 취급하지 않는다.
- 비즈니스 데이터 조회와 mutation은 외부 API server 호출을 우선한다.
- SSR에서 auth 해석, 보호 라우트 판단, 요청 단위 API 전달 같은 로직이 필요할 때만 TanStack Start server를 사용한다.
- SSR 레이어는 얇고 명시적으로 유지한다.
...
### 배치 판별 기준: `entities` vs `components` vs `features`

어디에 둘지 애매할 때는 아래 질문으로 판별한다.

- 여러 흐름에서 재사용될 수 있는 도메인 명사 표현이 중심인가? 그러면 `entities`
- 유스케이스, workflow, mutation, form, search, filter, route 주도 동작이 들어가나? 그러면 `features`
- 도메인 동작 없이 재사용 가능한 공용 UI인가? 그러면 `components`
...
- 필드 단위 validation 에러는 해당 입력 요소 근처의 인라인 UI에 표시한다
- 로그인 같은 폼의 인증 실패는 toast가 아니라 폼 상단의 인라인 UI로 처리한다
- 네트워크 오류나 알 수 없는 mutation 실패는 일시적 피드백으로 `sonner` toast를 사용한다
- 프로젝트 표준 피드백이 `sonner`로 정해져 있다면 seed 내부 전용 toast 구현에 의존하지 않는다
...
`VITE_AUTH_MODE`는 외부 API의 두 가지 인증 계약을 상정한다.

- `cookie`: 외부 API가 인증 쿠키를 설정하고 프론트엔드는 그 쿠키를 재사용한다
- `bearer`: 외부 API가 응답 바디에 `accessToken`, `refreshToken`을 내려준다

현재 구현 기준:

- `cookie` 모드에서는 auth 요청이 프론트엔드 런타임에서 외부 API로 직접 간다
- `bearer` 모드에서는 auth 요청이 TanStack Start BFF handler를 거친다
- `bearer` 모드에서 브라우저는 `AUTH_SESSION_COOKIE_NAME`에 raw refresh token을 저장하지 않는다
- `AUTH_SESSION_COOKIE_NAME`에는 서버 세션 id가 들어가고, 실제 `accessToken` / `refreshToken`은 Redis 기반 서버 auth session store에 남는다

### 현재 인증 흐름 매트릭스

| 렌더링 모드 | `cookie` 모드                                                                                                                                                                                                                                                                                                                             | `bearer` 모드                                                                                                                                                                                                                                                                                                           |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CSR         | 브라우저가 `Query`/mutation을 통해 외부 auth API를 직접 호출한다. 쿠키 기반 인증 정보는 `withCredentials`로 외부 API에 전달된다. `me`는 auth query key에 캐시된다.                                                                                                                                                                        | 브라우저에서 시작한 auth 동작도 BFF server function을 통해 처리한다. 브라우저는 httpOnly 세션 쿠키만 보내고, BFF는 `AUTH_SESSION_COOKIE_NAME`에서 session id를 읽은 뒤 Redis에 저장된 서버 측 bearer session을 조회하고 필요한 헤더를 주입해서 외부 API를 호출한다. `me` 역시 `Query`에 캐시된다.                       |
| SSR         | `signin` / `signout` / `refresh`는 계속 브라우저에서 외부 API로 직접 호출한다. 대신 SSR 보호 라우트의 auth 해석이 필요할 때 TanStack Start가 들어온 요청의 cookie header를 읽고 서버에서 외부 `/auth/me`를 호출한다. 그 외 일반 데이터 요청은 route가 별도 server preload 경로를 두지 않는 한 보통 기존 브라우저 + `Query` 흐름을 따른다. | 브라우저의 auth 요청은 SSR에서도 먼저 같은 BFF 경로로 들어온다. TanStack Start는 `AUTH_SESSION_COOKIE_NAME`을 읽어 Redis에 저장된 서버 측 bearer session을 해석하고, `/auth/me` 같은 access token 기반 요청에는 `Authorization` 헤더를, refresh 요청에는 `x-refresh-token` 헤더를, signout에는 두 헤더를 함께 사용한다. |

### Bearer Mode 메모

현재 bearer 구현은 SSR 지향 BFF auth 구조다.

- 로그인 시 서버가 외부 API login endpoint를 호출한다
- 서버는 `accessToken`, `refreshToken`, `tokenType`, 만료 시각을 Redis 기반 서버 auth session store에 저장한다
- 브라우저에는 생성된 session id가 담긴 httpOnly 쿠키만 내려간다
- 로그인 이후의 auth 요청은 쿠키에 담긴 session id를 기준으로 Redis 기반 서버 auth session store에서 토큰 정보를 꺼내 사용한다
- `/auth/me`는 저장된 access token을 `Authorization` 헤더에 담아 서버에서 해석한다
- `/auth/me`가 `401`을 반환하면 서버가 `x-refresh-token`으로 refresh를 수행하고 Redis 세션을 갱신한 뒤 다시 시도한다
- 로그아웃 시 upstream auth 상태와 로컬 Redis auth session을 함께 정리한다

즉 현재 bearer 모드는 사용자의 동작이 CSR UI에서 시작되더라도, 실제 인증 해석은 SSR + BFF 구조를 중심으로 설계되어 있다.

### Redis Session Runtime

bearer 모드는 Redis 기반 auth session store를 전제로 한다.
...
권장 env 값은 프론트 서버가 어디서 도는지에 따라 달라진다.

- 앱은 호스트에서 실행되고 Redis만 Docker에서 돌면 `REDIS_HOST=127.0.0.1` 같은 host 접근 값을 사용한다
- 앱과 Redis가 같은 Docker 네트워크 안에서 함께 돌면 `REDIS_HOST=redis` 같은 서비스명을 사용한다

### SPA Mode 메모

TanStack Start는 `tanstackStart({ spa: { enabled: true } })` 형태의 SPA mode도 지원한다.
src/routes/\_app/route.tsx ssr플래그 내리기
...
현재 프로젝트에서 다른 수정 없이 SPA mode만 켰을 때 영향 범위:

- 초기 라우트 렌더링이 SSR 기반 route resolution에서 hydration 이후 client route resolution 중심으로 바뀐다
- 현재 `_app` route `beforeLoad`와 `server/auth/requireUser`가 담당하는 SSR auth guard 경로는 더 이상 첫 요청의 주 인증 게이트가 아니게 된다
- `cookie` 모드는 지금의 SSR 보호 라우트 `/auth/me` 해석 경로를 잃고 client `Query` 기반 bootstrap 비중이 커진다
- `bearer` 모드는 BFF auth handler를 계속 사용할 수 있지만, 역할이 SSR 우선 auth 해석보다는 토큰 은닉과 refresh orchestration 쪽으로 옮겨간다
- 보호 페이지는 hydration 이후 인증이 판별되므로 redirect 전에 잠깐 pending client state가 보일 수 있다
- server function과 server route는 계속 사용할 수 있지만, 배포 시 server function 요청이 앱 서버로 정상 전달되도록 라우팅을 유지해야 한다

이 프로젝트를 SPA-only로 안전하게 바꾸는 권장 순서:

- `vite.config.ts`에서 SPA mode를 활성화한다
- route guard에서 현재 SSR 전용 auth 분기를 제거하고 client `ensureCurrentUser(...)`를 보호 라우트의 기본 판별 경로로 삼는다
- `bearer` 모드의 BFF auth handler는 유지해서 access/refresh token이 브라우저에 직접 노출되지 않게 한다
- auth mode 문서도 `cookie` 모드를 SSR 보조 auth 해석이 아니라 브라우저 중심 auth resolution 구조로 다시 정리한다
- hydration 시 auth flicker를 줄이기 위해 보호 라우트의 pending/loading UX를 명시적으로 추가한다
- 배포 라우팅에서 `/_serverFn/*` 같은 server-only endpoint가 계속 앱 서버로 전달되도록 확인한다
- 한 번에 SPA mode로 전환하는 대신, 필요하면 selective SSR 축소부터 단계적으로 적용하는 것도 고려한다
